...
BaSynthDefs {
	...
	*loadBrownFlute {
		...
		SynthDef(\flute, {| out = 0, freq = 440, voices=10, fade = 1, gate = 1, vol= 0.25, amp = 0.5, pan = 0|
			...
			Out.ar(out, Pan2.ar(wave*amp, pan)) //stereo, panned centre
		}).add;
	...
	*loadChaosCL {
		...
		SynthDef(\noiseL1, {|out = 0, freq = 220, alpha  = 1, beta = 1.9, xi = 0, amp = 0.1, cutoff = 3000, pan = 0, vol = 0.5, fadein = 1, fadeout = 1, gate = 1, shape = 4|
			...
			source = LeakDC.ar(SinOsc.ar(SinOsc.ar(freq*7, freq*6, freq/2), 0, amp)*Saw.ar(Saw.ar(freq*2, freq/0.5), amp)*WhiteNoise.ar(amp)+Saw.ar(freq/4, amp)*CuspL.ar(freq, alpha, beta, xi, amp)*0.2);
	...
	*loadFmSynth {
		...
		SynthDef(\fms, {|out = 0, gate = 1, vol = 0.0001, shape = 1,  carfreq = 440, modfreq = 440, ind = 444, freq = 440, decay = 0.02, cutoff = 2000, amp = 0.001, trig = 1, pan = 0|
         ...
         source = SinOsc.ar(SinOsc.ar(carfreq, modfreq, ind)+freq, 0, amp)*PinkNoise.ar(amp);
			...
			source = LeakDC.ar(LPF.ar(source, cutoff, amp)).softclip;
         Out.ar(out, Pan2.ar(source*env, pan))
         }).add;
...
Out.ar(out, Pan2.ar(source*amp, pan))
	...
	*loadSynthGrainA {
		...
		SynthDef(\grainAn, { |out = 0, gate = 1, carfreq = 1000, modfreq = 122, ind = 0.5, rate = 1, pos = 0.5, in = 1, bufnum,   trig = 1, dur = 1, amp = 1, sndbuf=0, envbuf = 1, cutoff = 6000, pan = 0, vol = 0.5, sample, shape = 1|
    var env,index, modulator, zoro, outbus, trigger;
		...
		trigger = LeakDC.ar(LPF.ar(GrainBuf.ar(2, Dust.kr(trig), dur, sndbuf, BufRateScale.kr(sndbuf)*(modulator/440)*rate, pos, 2, pan, envbuf), cutoff));
	...
	*loadSynthSines {
	...
	var env, source;
			env = EnvGen.kr(Env([fadeinvl, 0.8, fadeoutvl], [fade, fade]), doneAction: 2);
			//env = EnvGen.kr(Env.perc, doneAction:2);
			source = LeakDC.ar(SinOsc.ar([freq, freq2, freq3], 0, amp)+Saw.ar([freq, freq2, freq3], 0, amp)+FSinOsc.ar([freq*2, freq2*2, freq3*2], 0.5, amp)+SinOsc.ar(SinOsc.ar(freq*4, freq2*4, freq3*4), 0, amp));
